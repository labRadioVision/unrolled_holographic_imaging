...
|------|--------------|---------------------|--------|
| PEC (nowalls) | `run_wlista_synthetic_pec.py` | `E_total_Ken_PEC_nowalls.mat`, `E_total_freespace_nowalls.mat` | `synthetic_sets/` |
| Adipose tissue | `run_wlista_synthetic_adipose.py` | `E_total_Ken_grasso_nowalls.mat`, `E_total_freespace_nowalls.mat` | `synthetic_sets/` |
| Muscle tissue | `run_wlista_synthetic_muscle.py` | `E_total_Ken_muscoloso.mat`, `E_inc.mat` | `synthetic_sets/` |
| Real measurements | `run_wlista_measurements.py` | `empty_*.mat` (8 positions, Nov–Dec 2024) | `real_data/` *(released later)* |
...
python3 run_wlista_synthetic_pec.py --model wlista

# 2 — Adipose tissue (lossy dielectric body)
python3 run_wlista_synthetic_adipose.py

# 3 — Muscle tissue (high-permittivity body)
python3 run_wlista_synthetic_muscle.py

# 4 — Real indoor measurements (2.48 GHz), W-LISTA
python3 run_wlista_measurements.py --model wlista

# 4b — LoRaW-LISTA: two-stage W-first (low-rank adaptation of T)
python3 run_lorawlista_wfirst_measurements.py --rank 8
```
...
python3 run_wlista_synthetic_pec.py \
...
```
.
├── run_wlista_synthetic_pec.py           # PEC (nowalls) — LISTA / W-LISTA
├── run_wlista_synthetic_adipose.py       # adipose-tissue body — W-LISTA
├── run_wlista_synthetic_muscle.py        # muscle-tissue body — W-LISTA
├── run_wlista_measurements.py            # real measurements — LISTA / W-LISTA
├── run_lorawlista_wfirst_measurements.py # real measurements — LoRaW-LISTA (W-first)
├── requirements.txt
